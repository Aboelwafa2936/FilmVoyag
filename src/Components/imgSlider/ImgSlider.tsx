...
export default function ImgSlider({
  ...
  const isOnPeopleDetailsPage = currentPath.startsWith("/peopleDetails/");
    ...
    const refetchUserData = async () => {
      if (sessionId) {
        try {
          ...
          const watchlistMovies = await getData(
            sessionId,
            "movies",
            "watchlist"
          );
          ...
          setFavoriteItems(
            [...favoriteMovies, ...favoriteTVShows].map((item) => item.id)
          );
          setWatchlistItems(
            [...watchlistMovies, ...watchlistTVShows].map((item) => item.id)
          );
        } catch (error: any) {
          console.error(
  ...
  const movies = posterPaths.map((posterPath, index) => {
    const isMovie = !!movieDate[index];
    const isTV = !!tvDate[index];
    return {
      posterPath,
      ...
      date: isMovie ? movieDate[index] : isTV ? tvDate[index] : "",
      category: isMovie ? "movie" : isTV ? "tv" : "",
    };
  });

  const Category = movies.map((movie) => {
  ...
  return (
    <>
      ...
      <Slider {...settings} className="gap-x-4">
        {movies.map((movie) => {
          console.log(movie.category); // if it's in the person details it's alwayes a "movie"
          console.log(category); // if it's in the person details it's an array of both "movie" and "tv"
          ...
          return (
            <div key={movie.id} className="p-2">
              <div className="p-4 bg-slate-950 rounded-md relative">
                ...
                <Link
                  to={`/details/${
                    isOnPeopleDetailsPage ? movie.category : category
                  }/${movie.id}`}
                >
                  <img
                    data-category={
                      isOnPeopleDetailsPage ? (movieDate ? "movie" : "tv") : ""
                    }
